...
export default function MasterPremiumHero() {
  ...
  return (
    ...
    >
          ...
          >
            ...
            <div className="relative z-30 text-center px-6">
              ...
              <div className="flex gap-6 justify-center">
                <button className="px-12 py-5 bg-[#101828] text-white rounded-[2rem] font-bold text-lg hover:shadow-[0_10px_30px_rgba(0,212,126,0.3)] transition-shadow">Start Your Project</button>
              </div>
            </div>
          </motion.div>
        )}
        ...
        {current === 1 && (
          ...
          >
            ...
            <div className="relative z-30 flex flex-col items-center">
              ...
              <button className="px-10 py-4 bg-[#00D47E] text-white rounded-2xl font-bold text-lg shadow-xl hover:bg-[#00b36a] transition-colors">Explore Ecosystem</button>
            </div>
          </motion.div>
        )}
          ...
          >
            ...
            <div className="relative z-20 text-center px-6">
              ...
              <div className="flex gap-5 justify-center">
                <button className="px-10 py-5 bg-gray-900 text-white rounded-2xl font-bold text-lg flex items-center gap-2 group">
      ...
      <div className="absolute bottom-12 w-full opacity-30 overflow-hidden z-40">
        <motion.div animate={{ x: [0, -1000] }} transition={{ duration: 30, repeat: Infinity, ease: "linear" }} className="flex gap-20 whitespace-nowrap px-10">
          {Array(10).fill("EAMMU SOLUTIONS").map((t, i) => (
            <span key={i} className="text-4xl font-black italic tracking-tighter text-transparent border-b-2 border-[#00D47E] bg-clip-text bg-gradient-to-r from-gray-400 to-gray-200">{t}</span>
          ))}
        </motion.div>
      </div>
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex gap-3 z-50">
        {slides.map((_, i) => (
          <button key={i} onClick={() => setCurrent(i)} className={`h-1.5 transition-all duration-500 rounded-full ${current === i ? "w-8 bg-[#00D47E]" : "w-2 bg-gray-300"}`} />
        ))}
      </div>
    </section>
  );
