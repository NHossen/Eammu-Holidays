...
export default function BottomCarousel() {
  ...
  // ✅ track whether user is manually scrolling — pause auto-advance
  ...
  // ✅ auto-advance — pauses when user touches/drags
  useEffect(() => {
  ...
  const handleInteractionEnd = () => {
    ...
    }, 3000); // resume 3s after user stops
  };
  ...
  return (
    <div className="bg-white py-10 px-4">
      <div className="max-w-7xl mx-auto relative">
        <div
          ref={bottomRef}
          // ✅ no framer-motion — pure CSS scroll snap
        ...
        >
          {bottomSlides.map((slide, index) => (
            <div
              key={slide.id}
              // ✅ CSS hover scale replaces motion.div whileHover
            ...
            >
              <Link href={slide.link || '#'} className="block w-full h-full relative">
                <Image
                  ...
                  fill
                  // ✅ object-cover not object-fill — prevents distortion
                  className="object-cover transition-all duration-700 group-hover:blur-sm group-hover:scale-110"
                  // ✅ below the fold — all lazy except first 3 visible on desktop
