...
const VisaServices = () => {
  ...
  // ✅ null guard + useCallback
  const scroll = useCallback((direction) => {
    visaRef.current?.scrollBy({ left: direction === 'left' ? -340 : 340, behavior: 'smooth' });
  }, []);
      ...
      >
        ...
        >
          {visaData.map((visa) => (
            ...
            >
              {/* ✅ next/image — WebP, srcset, lazy, no layout shift */}
        ...
        <button
          ...
          onClick={() => scroll('left')}
          className="absolute cursor-pointer -left-4 top-1/2 -translate-y-1/2 z-10 bg-white p-2 rounded-full shadow-lg opacity-0 group-hover:opacity-100 transition hidden lg:block border border-gray-100"
        >
        ...
        <button
          ...
          className="absolute cursor-pointer -right-4 top-1/2 -translate-y-1/2 z-10 bg-white p-2 rounded-full shadow-lg opacity-0 group-hover:opacity-100 transition hidden lg:block border border-gray-100"
        >
