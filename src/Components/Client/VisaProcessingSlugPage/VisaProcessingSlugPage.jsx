...
function ReverseCalculator({ min, max, unit }) {
  ...
  const urgency = useMemo(() => {
    ...
    if (daysLeft < 3)  return { label:`🔴 Apply within ${daysLeft} days — urgent!`,  color:"text-orange-700 bg-orange-50 border-orange-200"};
    if (daysLeft < 14) return { label:`🟡 Apply within ${daysLeft} days — soon`,     color:"text-amber-700 bg-amber-50 border-amber-200"  };
    ...
    <div className="min-h-screen bg-white font-sans my-20">
      ...
      <div className="bg-slate-50 border-b border-slate-100 px-6 py-3">
        <div className="container mx-auto max-w-6xl flex items-center gap-2 text-xs text-slate-400 font-semibold flex-wrap">
          ...
          <Link href="/travel-resources/visa-processing-time-tracker" className="hover:text-[#005a31] transition-colors">Visa Processing Time Tracker</Link>
          <ChevronRight size={12} />
          ...
          <div className="space-y-6">
            ...
            {RELATED.length > 0 && (
              <div className="bg-white border-2 border-slate-100 rounded-2xl p-6">
                <h4 className="font-black text-sm text-slate-800 uppercase tracking-wider mb-4">Other Visa Types for {countryData.name}</h4>
                <div className="space-y-3">
                  {RELATED.map(([k, v]) => (
                    <Link
                      key={k}
      ...
      <section className="bg-[#005a31] py-16 px-6 text-center">
        <div className="container mx-auto max-w-3xl">
          ...
          <p className="text-green-200/70 font-medium mb-8">Search 195+ countries — E-Visa, Sticker, Transit & complex cases</p>
          <Link
